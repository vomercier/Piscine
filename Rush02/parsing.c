...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2021/03/28 08:23:24 by vmercier          #+#    #+#             */
/*   Updated: 2021/03/28 09:22:40 by vmercier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*find_word_in_dict(int fd, int nb_char, char buffer) 
{
...
char	*ft_remove_multiple_space(char *str) 
{
	...
	str_cpy[j] = '\0';
	ft_realloc(&str_cpy, '-');//a checker pour voir si utile ou non
	return (str_cpy);
}
...
char	*get_word(char *str) 
{
	...
	while (read(fd, &buffer, 1))
	{
		...
		if ((buffer == ':' || buffer == ' ') && str[i] == '\0')
			output = find_word_in_dict(fd, i, buffer); 
		if (str[i] == '\0')
		{
			ft_go_to_next_line(&fd, &i);
			continue;
...
char	*get_word2(char *s1, char *s2)
{
	char *str;
	char *result;

	str = ft_strcat(s1, s2);
	result = ft_get_word(str);
	return (result);
}
